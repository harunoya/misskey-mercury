...
description: "Misskey の .Codex/ ハーネス (skills/agents/commands) を 7 カテゴリで採点する確定的な監査。"
---
...
Imported into Misskey .Codex/ on 2026-05-10.
...
note: 元 ECC 版は scripts/harness-audit.js (専用 Node スクリプト) で機械採点していたが、Misskey は ECC plugin runtime に依存しない方針なので、Codex が直接ファイルを読んで採点する手動運用版に書き換えた。Misskey 固有の重要観点 (SPDX 適用率 / endpoint-list 登録漏れ / migration 順序 / ja-JP.yml 整合) を評価軸として明示的に組み込んでいる。
-->

# /harness-audit — Misskey ハーネス監査

Misskey リポジトリの `.Codex/` 構成を 7 カテゴリで採点し、改善優先度を提示する。
...
| 2 | Context Efficiency | frontmatter description の冗長度、SKILL.md の長さ分布、重複情報、AGENTS.md の肥大化 |
| 3 | Quality Gates | 変更ファイル lint、`/quality-gate`、変更別 test / typecheck、CI gate との整合 |
| 4 | Memory Persistence | `.Codex/skills/*/SKILL.md` と `references/` の同期状態を評価。プロジェクト側 `.Codex/memory/` は未採用方針 (auto-memory はユーザーホーム側で自動運用) のため、ここを採点起点にせず既定 5/10 から開始する |
...
3) [Cost Efficiency] enabledPlugins から未使用のものを外す。
   `.Codex/settings.json` の `enabledPlugins` と実プロジェクト利用状況を照合。
...
- 確定的: 同じ commit / 同じ `.Codex/` 構成なら同じスコア
